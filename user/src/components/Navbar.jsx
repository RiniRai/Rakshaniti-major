...
const ThemeToggle = React.forwardRef(
  ({ darkMode, setDarkMode, ...props }, ref) => {
    return (
      <button
        ref={ref}
        {...props}
      ...
      >
        {/* Sun */}
        <span className="absolute left-1 text-yellow-300 text-lg select-none pointer-events-none">
          ☀️
        </span>

        {/* Moon */}
        <span className="absolute right-1 text-gray-200 text-lg select-none pointer-events-none">
          🌙
        </span>
...
const Navbar = ({ isAuthenticated, setIsAuthenticated }) => {
  ...
  <Link to="/ask-Rakshaniti" className={`hidden lg:block ${navLinkClasses}`}>{t('navigation.askAI')}</Link>
      ...
      {isMenuOpen && (
        <div className="absolute left-0 w-full bg-orange-900 dark:bg-gray-900 top-20 lg:hidden shadow-xl animate-in slide-in-from-top-4 transition-colors duration-500">
          <div className="flex flex-col p-4 gap-y-2">
            ...
            <Link to="/ask-Rakshaniti" className="block px-4 py-3 rounded-md hover:bg-orange-800/70 dark:hover:bg-gray-700/50" onClick={toggleMenu}>{t('navigation.askAI')}</Link>
